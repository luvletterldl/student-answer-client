...
export async function uploadFaceCheckImageToAliOss(examStudentId, examId, filePath) {
  const option = {
    // url: `${Main.host}api/k12/wx/${classId}/${examRecordDataId}/uploadImageToAliOss`,
    url: `${Main.host}/api/k12/upload/uploadImageToAliOss`,
    filePath: filePath,
    header,
    formData: {
      type: `faceCheck/${examStudentId}_${examId}`,
      type, filePath
    },
    name: 'image',
  }
...
export async function compareFaceSync(fileUrl, examStudentId, examId) {
  const option = {
    url: `${Main.host}api/k12/ecs_oe_face/examCaptureQueue/compareFaceSync`,
    method: 'POST',
    header,
    data: {
      fileUrl, examStudentId, examId
    }
  }
  return formatErrOrRes(await uni.request(option))
}
